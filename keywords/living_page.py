...
class LivingPage(BasePage):
    ...
    def start_spider(self, platform, url):
        self.click(self.spider_platform, (1, 1))
        self.click(self.platform_text.format(platform), (1, 1))
        self.input(self.platform_url, url)
        self.click('//span[text()="开启"]', (1, 1))
        logger.info('开启爬虫成功')

    def mon_answer(self):
        while True:

            if self.find_element_by_xpath('//div[@class="reply-state"]'):
                logger.info('爬虫正常使用')
                break
            else:
                time.sleep(1)
    ...
    def shutdown_living(self):
        self.click('//*[text()="结束直播"]', (1, 1))
        self.click('//*[text()="是"]', (1, 1))
